...
def iset_bfs_3_coloring(G):
    maximal = get_maximal_isets(G)

    for iset in maximal:
        color = bfs_2_coloring(G, precolored_nodes=iset)
        if color is not None:
            return color  # success: S is color 2, others are 1 and 3

    # None worked and thus no 3-coloring
    G.reset_colors()
    return None
